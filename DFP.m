...
function[X, H, F, G,HN] = DFP(f,H0,x0,m,k)
    ...
    % 初始值要求出来带入
    H(:,:,1) = inv(H0);  % 注意这里的初始值是直接带入H0的逆矩阵的(DFP算法)  
    HN(:,:,1) = inv(H0);
    X(:,1) = x0;
    F(1,1) = subs(f,c,{X(:,1)'});
    ...
    % DFP 要计算初始的x2
    i = 1;
    X(:,i+1) = X(:,i) - (H(:,:,i))*G(:,i);  % x2也不计算逆矩阵，不过注意这里的H1实际上是x0出海森矩阵的逆
    F(1,i+1) = subs(f,c,{X(:,i+1)'}); 
    G(:,i+1) = subs(g,c,{X(:,i+1)'});
    
    for n = 2:k+1
        qn = G(:,n) - G(:,n-1);  % 计算DFP中的qk
        sn = X(:,n) - X(:,n-1);  % 计算DFP中的qk
        H(:,:,n) = H(:,:,n-1) + (sn*sn')/(qn'*sn) - (H(:,:,n-1)*sn*sn'*H(:,:,n-1)')/(qn'*H(:,:,n-1)*qn);  % 从x2起的海森矩阵是通过迭代计算的
        HN(:,:,n) = inv(H(:,:,n));  % 存入x2起的海森矩阵的逆
        
        X(:,n+1) = X(:,n) - (H(:,:,n))*G(:,n);  % 注意DFP算法是不需要求逆矩阵的
